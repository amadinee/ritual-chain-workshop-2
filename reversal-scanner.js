...
class ReversalScanner extends EventEmitter {
  constructor() {
    super();
  ...
  createContract(params) {
    const { 
      asset,
      reversalType, // 'TOP', 'BOTTOM', 'WHIPSAW'
      threshold,
      feedUrl,
      jsonPath,
      duration,
    ...
    const contractId = `REV_${++this.idGenerator}`;
    ...
    console.log(`✅ Reversal Contract ${contractId} created!`);
    console.log(`   Asset: ${asset}, Type: ${reversalType}, Threshold: ${threshold}`);
  ...
  async settleContract(contractId) {
    ...
    console.log(`🔍 Scanning for reversals in ${contractId}...`);
    ...
    try {
      ...
      if (contract.priceHistory.length >= 4) {
        ...
        if (contract.reversalType === 'TOP') {
          // Look for peak and subsequent decline
          const peak = Math.max(...recent);
          const peakIndex = recent.indexOf(peak);
          if (peakIndex < recent.length - 1 && peakIndex > 0) {
            const decline = (peak - last) / peak;
            reversalStrength = decline;
          }
        } else if (contract.reversalType === 'BOTTOM') {
          // Look for trough and subsequent rise
          const trough = Math.min(...recent);
          const troughIndex = recent.indexOf(trough);
          if (troughIndex < recent.length - 1 && troughIndex > 0) {
            const rise = (last - trough) / trough;
            reversalStrength = rise;
          }
        } else if (contract.reversalType === 'WHIPSAW') {
          // Look for multiple direction changes
          ...
          reversalStrength = directionChanges / (changes.length);
        }
        contract.reversalStrength = reversalStrength;
        console.log(`   Reversal strength: ${reversalStrength.toFixed(4)}`);
      }

      // Determine outcome based on reversal type
      let resolved = false;
      switch (contract.reversalType) {
        case 'TOP':
          resolved = reversalStrength > contract.threshold;
          break;
        case 'BOTTOM':
          resolved = reversalStrength > contract.threshold;
          break;
      ...
      console.log(`✅ Contract ${contractId} settled as ${contract.outcome} (strength: ${reversalStrength.toFixed(4)})`);
      ...
      this.emit('contractSettled', {
        ...
        reversalStrength: contract.reversalStrength,
        reversalType: contract.reversalType,
...
module.exports = { ReversalScanner };
